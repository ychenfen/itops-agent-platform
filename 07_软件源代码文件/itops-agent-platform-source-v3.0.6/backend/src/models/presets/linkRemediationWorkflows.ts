...
export function linkRemediationWorkflows(): void {
  ...
  const extraPolicies = [
    // ===== 审批型：高风险操作需要人工确认 =====
    {
      ...
      workflow_id: fullFlowId || faultDiagId,
    ...
    // ===== 日志异常 → 自动诊断 + 日志轮转 =====
    {
      ...
      execution_mode: 'auto',
      workflow_id: fullFlowId || logAnalysisId,
      workflow_params: JSON.stringify({
        server_id: '{{alert.host}}',
    ...
    // ===== 服务批量重启（审批模式） =====
    {
      ...
      execution_mode: 'approval',
      workflow_id: fullFlowId || changeExecId,
      workflow_params: JSON.stringify({
        server_id: '{{alert.host}}',
        restart_all: true,
    ...
    // ===== Zabbix 系统告警（时间、温度、UPS等） =====
    {
      ...
      execution_mode: 'auto',
      workflow_id: fullFlowId || healthCheckId,
      workflow_params: JSON.stringify({
        server_id: '{{alert.host}}',
    ...
    // ===== 兜底：未匹配告警（来源不限，级别不限） =====
    {
      ...
      execution_mode: 'suggestion',
      workflow_id: fullFlowId || alertHandleId || changeExecId || faultDiagId || fallback,
      workflow_params: JSON.stringify({
