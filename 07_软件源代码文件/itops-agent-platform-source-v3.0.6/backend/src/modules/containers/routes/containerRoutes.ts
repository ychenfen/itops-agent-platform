...
router.get('/', async (req: Request, res: Response) => {
  if (!dockerService.isAvailable() && !req.query.endpointId) {
    return res.json({
      success: true,
      data: [],
      total: 0,
      available: false,
      message: 'Docker 服务未连接，请在端点页配置 Docker 主机',
    });
  }
...
router.get('/images/list', async (req: Request, res: Response) => {
  if (!dockerService.isAvailable() && !req.query.endpointId) {
    return res.json({ success: true, data: [], available: false, message: 'Docker 服务未连接' });
  }
  if (!checkDockerAvailable(res, req)) return;
  try {
    const d = getDocker(req);
