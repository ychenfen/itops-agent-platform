 ...
 * 解析对外上报的版本号。
 *
 * npm_package_version 只在经由 npm script 启动时存在；systemd 直接执行
 * `node dist/app.js` 时该变量为空，健康接口会退回占位值并与交付文档中的版本号不符。
 * 因此优先读包描述文件，环境变量仅作为兜底。src 与 dist 下本文件到 package.json
 * 的相对层级一致，同一路径在两种运行方式下均可用。
 */
